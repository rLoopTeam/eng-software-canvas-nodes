...
def main():
	...
	canvas.print_out('Standard outlet started')

	while 1:

		msg_id, msg_data = canvas.recv(receiver)
		
		if (msg_id == ml.messages['so_temp_req']['id']):
			temp_data = randint(30,40)
			canvas.send(sender, ml.messages['so_temp_data']['id'], temp_data) #send random temp data
